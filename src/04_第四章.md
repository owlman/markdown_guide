...
指定字体的方式很简单，我们只需要以`{\字体{Sample}}`的形式使用上述字体标记，就可以将“Sample”这几个字符设置为指定字体了。除此之外，在必要情况下，我们还可以使用`\color`标记来指定这些字体的颜色，该标记的第一个参数为颜色的名称，第二个参数是被指定颜色的字符，譬如`$\color{black}{text}$`，就是将“text”这几个字符设置为`black`这个颜色（当然，这些颜色主要是面向Web浏览器的，对印刷品没有多大的意义）。在这里，可供我们选择的颜色如下：
