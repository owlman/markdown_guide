...
如果没有看到类似的版本信息，那就说明你需要先自己安装一下Node.js运行环境，然后才能继续接下来的学习。我们在附录C中提供了一篇介绍如何在各主流操作系统上安装Node.js的简易教程，读者可自行参考。在确认了Node.js运行环境之后，我们就可以通过Node.js的包管理器npm来安装gitbook了，具体命令如下：
...
下面，我们先来架构一个基本的Hexo博客。首先，我们需要安装Hexo的命令行工具。当然，在安装该命令行工具之前，务必要记得先确认自己的计算机中是否已经安装了git和Node.js（尚未安装Node.js的朋友，可以参考附录C中的教程自行安装该运行环境）。然后，我们就可以使用`npm`包管理器安装Hexo了，具体命令如下：
...
然后，再将`_config.yml`文件中的`theme`参数设置为`Anisina`即可。现在，你是不是迫不及待想看看自己的博客是什么样子了？不要着急。为了让呈现效果更好一点，我们还需在加入一篇自己的文章。在Hexo博客中，新文章可以使用`hexo new`命令来添加，譬如我们想添加一篇书评的话，就可以执行如下命令：

![添加新文章](img/6-21.png)

如你所见，我将之前写的一篇关于《三体》的书评添加到了博客中。现在，我们可以再次将博客运行起来，看看个性化配置的效果了。请注意，在重新生成博客之前，最好先使用`hexo clean`命令清除一下之前生成的结果。具体来说，就是在项目目录下打开命令行终端，依次输入下面三个命令：
...
当然，我们也可以看到这个博客的主题还有些需要设置的地方，譬如导航栏，标题配图之类的。但这些内容需要我们根据自己所选主题的项目文档来做设置，譬如对于我在这里所选的这款主题，就要去`https://github.com/haojen/hexo-theme-Anisina`这个项目去寻找相关的设置说明，这里就不多做说明了，毕竟每个人会选择的主题都不一样。
